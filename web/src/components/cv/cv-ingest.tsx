...
export function CvIngest({ onSaved }: { onSaved?: () => void }) {
  ...
  const runStream = useCallback(async (init: RequestInit) => {
    setPhase("parsing");
    setTrace("Lecture de votre CV…");
    setErr("");
    try {
      const r = await fetch("/api/cv/ingest", init);
      if (r.status === 404) {
        setErr("Connectez d'abord un CLI d'IA dans la configuration — c'est lui qui analyse votre CV en local.");
        setPhase("error");
        return;
      }
      if (!r.body) {
        setErr("Aucune réponse.");
        setPhase("error");
        return;
      }
      ...
      for (;;) {
        ...
        if (parsed.error) {
          setErr(parsed.error === "unreadable" ? "Je n'ai pas pu extraire de texte de ce fichier (c'est peut-être une image scannée). Collez plutôt le texte." : "Impossible d'analyser le CV — collez plutôt le texte.");
          setPhase("error");
          return;
        }
        if (parsed.trace) setTrace(parsed.trace.split("\n").filter(Boolean).slice(-1)[0] || "Lecture de votre CV…");
      ...
      if (!final.markdown.trim()) {
        setErr("Aucun CV n'a pu être lu là-dedans — collez plutôt le texte.");
        setPhase("error");
        return;
      }
      ...
      setErr(e instanceof Error ? e.message : "erreur de flux");
      setPhase("error");
    }
  }, []);
  ...
  const ingestFile = (file: File) => {
    ...
    if (/\.(md|markdown|txt)$/i.test(file.name)) {
      file
        ...
        .then((t) => {
          if (!t.trim()) {
            setErr("Ce fichier semble vide — collez plutôt votre CV.");
            setPhase("error");
            return;
          }
          setMd(t.trim());
        ...
        .catch(() => {
          setErr("Impossible de lire ce fichier — collez plutôt votre CV.");
          setPhase("error");
        });
      return;
    }
  ...
  const save = async () => {
    if (!md.trim()) {
      setSaveErr("Votre CV semble vide — recollez-le.");
      return;
    }
    setSaveErr("");
    setPhase("saving");
    try {
      ...
      if (!r.ok) {
        ...
        setSaveErr(d.error || "Impossible d'enregistrer votre CV — réessayez.");
        setPhase("review"); // keep the parsed CV so they don't lose it
        return;
      }
    } catch {
      setSaveErr("Impossible d'enregistrer votre CV — vérifiez votre connexion et réessayez.");
      setPhase("review");
      return;
    }
    onSaved?.();
      ...
      <div className="space-y-3">
        ...
        >
          <textarea
            ...
            placeholder="Collez votre CV ici — ou déposez un PDF / fichier .md ci-dessous. Même un collage brut suffit ; on le nettoie."
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3 border-t border-border pt-3">
            ...
            >
              <Upload className="size-3.5" /> Téléverser un PDF / fichier
            </button>
            ...
            <span className="inline-flex items-center gap-1 text-[11px] text-faint">
              <Lock className="size-3" /> Reste sur votre machine. Analysé par votre propre IA.
            </span>
            <button
              type="button"
              disabled={!paste.trim()}
            ...
            >
              Lire mon CV <ArrowRight className="size-4" />
            </button>
          </div>
        </div>
        {phase === "error" &&
          (err === "needs-cli" ? (
            <div className="flex flex-wrap items-center gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2.5 text-[13px] text-amber-700 dark:text-amber-300">
              ...
              <span>Pour lire un PDF, connectez votre CLI d&apos;IA — ou collez le texte de votre CV ci-dessus (aucune configuration).</span>
              <Link href="/config" className="ml-auto inline-flex items-center gap-1 rounded-md bg-amber-500/20 px-2.5 py-1 font-medium text-amber-700 transition hover:bg-amber-500/30 dark:text-amber-200">
                Connecter votre CLI d&apos;IA <ArrowRight className="size-3.5" />
              </Link>
            </div>
          ) : (
    ...
    return (
      <div className="rounded-2xl border border-border bg-surface/60 p-6 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <span className={`${instrumentSerif.className} text-lg text-foreground`}>{trace || "Lecture de votre CV…"}</span>
        </div>
        <div className="mt-3 inline-flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 text-[11px] font-semibold text-emerald-600 dark:text-emerald-400">
          <span className="size-1.5 rounded-full bg-emerald-500" /> 0 jeton · 0,00 $ · en local
        </div>
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface/60 p-4 backdrop-blur-sm md:p-5">
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <h3 className={`${instrumentSerif.className} text-lg text-foreground`}>Voici votre CV — relisez et enregistrez</h3>
        {readiness && (
          <span
            className={cn(
          ...
          >
            ...
            {readiness.scoreable ? "Prêt à être comparé" : "Un peu léger"}
          </span>
        )}
      </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        >
          ...
          Enregistrer et trouver mes offres
        </button>
        <button
          type="button"
          onClick={() => {
        ...
        <span className="ml-auto inline-flex items-center gap-1 text-[11px] text-faint">
          <Lock className="size-3" /> Enregistré en local dans cv.md
        </span>
      </div>
    </div>
  );

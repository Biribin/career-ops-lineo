...
function extractWhy(job: Job): string {
  ...
  const para = body
    ...
    .find((p) => p.length > 60 && /\b(you|your|fit|match|strong|experience|background|vous|votre|profil|adéquation|expérience|parcours)\b/i.test(p));
  return para ? para.slice(0, 240) : "Vous correspondez bien à ce poste — ouvrez le rapport complet pour le détail.";
}
...
export function FirstScoreView() {
  ...
  // Doit rester aligné sur les titres posés par startJob (« Évaluation · X »).
  // L'ancienne forme anglaise est conservée pour les traitements déjà en localStorage.
  const company = firstDone.title.replace(/^(Évaluation|Évaluer|Evaluate)\s*·\s*/i, "");
  ...
  return (
    <div className="co-aha" role="dialog" aria-modal="true" aria-label="Votre première évaluation" onClick={close}>
      <style>{STYLE}</style>
      <div ref={panelRef} className="co-aha__card" onClick={(e) => e.stopPropagation()}>
        <div className="co-aha__glow" />
        <button onClick={close} aria-label="Fermer" className="absolute right-3 top-3 z-10 rounded-md p-1.5 text-faint transition hover:text-foreground">
          <X className="size-4" />
        </button>
        ...
        <div className="relative px-7 pb-7 pt-8">
          ...
          <div className="mt-4 flex items-start gap-4">
            ...
            {score != null && (
              <div className="shrink-0 text-right">
                ...
                <div className="text-[11px] uppercase tracking-wide text-faint">/ 5 d&apos;adéquation</div>
              </div>
            )}
          </div>
          ...
          {legit && (
            ...
            >
              <ShieldCheck className="size-3" /> Légitimité : {legit}
            </div>
          )}

          <p className="mt-5 flex items-center gap-1.5 text-[12px] text-faint">
            <Coins className="size-3.5" /> Cela a tourné sur votre propre IA. Tout ce qui précède — trouver cette offre — était gratuit.
          </p>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            >
              <FileText className="size-4" /> Voir le rapport complet
            </button>
            <button
              onClick={() => {
                close();
            ...
            >
              <Compass className="size-4" /> En trouver d&apos;autres comme ça
            </button>
          </div>
        </div>
      </div>

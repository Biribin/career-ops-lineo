...
export function DiscoveryCard({
  ...
  const statusLabel = WORKER_LABEL[job?.kind ?? ""] ?? "Traitement…";
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
        ...
        {unverified && (
          <span
            ...
            title="Trouvée par l'IA sur le web public — impossible de confirmer qu'elle est toujours ouverte sans l'ouvrir. L'évaluation lance une vraie vérification navigateur et tranche."
          >
            <ShieldQuestion className="size-3" /> non vérifiée
          </span>
        )}
        {offer.matchedKeyword && (
          <span className="text-faint" title="Correspondance de mot-clé — pas encore évaluée. Lancez une évaluation pour obtenir un score d'adéquation.">
            · correspond à <span className="text-brand/80">{offer.matchedKeyword}</span>
          </span>
        )}
      </div>
      ...
      <div className="mt-0.5">
        {evaluatedN || doneEval ? (
          ...
          <div className="inline-flex w-full items-center justify-center gap-1.5 rounded-md border border-brand/30 bg-brand-soft/60 px-2.5 py-2 text-xs font-medium text-brand">
            ...
            {statusLabel}
            <span className="text-brand/60">· dans les candidatures</span>
          </div>
        ) : (
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {isAdded ? "Dans les candidatures" : "Ajouter aux candidatures"}
            </button>
            <button
              type="button"
              onClick={evaluate}
              title={unverified ? "Lance une vraie évaluation — et vérifie que l'offre est toujours ouverte. Consomme des jetons." : "Lance une vraie évaluation notée. Consomme des jetons."}

...
export function ShortlistTray({
  ...
  const costText = estimate.tokens
    ? `≈ ${fmtTokens(estimate.tokens)} jetons${estimate.usd != null ? ` · ≈ ${estimate.usd.toFixed(2)} $` : ""}`
    : "consomme vos jetons";
        ...
        <div className="border-t border-border bg-surface shadow-lg shadow-black/10 sm:rounded-2xl sm:border">
          ...
          <div className="flex items-center gap-3 px-3 py-2.5 sm:px-4">
            ...
            >
              ...
              Sélection <span className="tabular-nums text-brand-text">({n})</span>
            </button>
          ...
          <div className="flex items-center gap-2 border-t border-border/60 px-3 py-1.5 text-[11px] text-muted sm:hidden">
            ...
            <span>{costText} — la seule étape payante</span>
          </div>
        </div>
      </div>
    </div>
  ...
  if (!hasCli) {
    return (
      <div className="flex items-center gap-2 text-xs">
        <span className="text-muted">Aucune IA configurée.</span>

...
export function DeleteFromTracker({ n }: { n: string }) {
  ...
  async function openConfirm() {
    ...
    try {
      ...
      if (!r.ok) {
        setErr(d.error || "Cette ligne ne peut pas être supprimée.");
        return;
      }
      setOrphan(d.orphanReport ?? null);
    } catch {
      setErr("Impossible de joindre le tracker.");
    }
  }
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      const d = await r.json().catch(() => ({}));
      if (!r.ok) {
        setErr(d.error || "La suppression a échoué.");
        setBusy(false);
        return;
      }
  ...
  if (!open) {
    return (
      ...
      >
        <Trash2 className="size-3.5" /> Retirer du tracker
      </button>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-red-400/30 bg-red-500/[0.06] p-3 text-xs">
      <p className="font-medium text-foreground">Supprimer définitivement la candidature n° {n} de votre tracker ?</p>
      <p className="mt-1 text-muted">
        C’est irréversible.{orphan ? ` Son fichier de rapport (${orphan}) reste sur le disque.` : ""}
      </p>
      ...
      <div className="mt-2.5 flex gap-2">
        ...
        >
          {busy ? <Loader2 className="size-3.5 animate-spin" /> : <Trash2 className="size-3.5" />} Supprimer
        </button>
        <button
          disabled={busy}

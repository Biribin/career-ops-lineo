...
export async function driveSession(
  ...
  if (!resolved || cliId !== "claude") {
    return { reached: false, turns: 0, reason: "Le pilotage agentique nécessite actuellement Claude Code (CLI capable de piloter un navigateur).", steps };
  }
  const shot = async () => {
    try {
    ...
    try {
      ...
      if (act.action === "click" && loc) {
        ...
        if (SUBMIT_RX.test(txt)) {
          note = "a refusé de cliquer sur un bouton d'envoi (c'est l'humain qui envoie)";

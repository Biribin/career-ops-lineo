...
1. Run \`node verify-portals.mjs --add "${input}"\` — it probes Greenhouse/Ashby/Lever/SmartRecruiters/WelcomeKit for the company's correct ATS slug and prints the suggested ats + slug AND the exact \`careers_url:\` line to write. Use that URL verbatim; do not rebuild it from the slug.
2. Open portals.yml, find the "${input}" entry under tracked_companies, and update its careers_url (and any api/slug field) to the suggested WORKING ATS URL. If that entry has \`enabled: false\`, set it to \`true\` (and drop its \`en_attente_ats: true\` line, if any) — otherwise verify-portals skips it entirely and step 3 can never pass. If its \`notes:\` still ask to complete careers_url or to enable the entry (e.g. "Compléter careers_url puis passer enabled: true"), remove that instruction from the note — a note contradicting the entry's real state misleads whoever reads it next, human or agent. Keep the rest of the note. Change ONLY this one company; preserve all other YAML structure, comments and formatting exactly.
